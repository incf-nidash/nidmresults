...
from nidmresults.objects.constants import (
    ...
    NIDM_CLUSTER_LABELS_MAP,
    NIDM_CLUSTER_SIZE_IN_RESELS,
    NIDM_CLUSTER_SIZE_IN_VOXELS,
    NIDM_COORDINATE,
    NIDM_COORDINATE_VECTOR,
    NIDM_COORDINATE_VECTOR_IN_VOXELS,
    NIDM_DISPLAY_MASK_MAP,
    NIDM_EQUIVALENT_THRESHOLD,
    NIDM_EQUIVALENT_ZSTATISTIC,
    NIDM_EXCURSION_SET_MAP,
    NIDM_EXPECTED_NUMBER_OF_CLUSTERS,
    NIDM_EXPECTED_NUMBER_OF_VOXELS_PER_CLUSTER,
    NIDM_EXTENT_THRESHOLD,
    NIDM_HAS_ALTERNATIVE_HYPOTHESIS,
    NIDM_HAS_CLUSTER_LABELS_MAP,
    NIDM_HAS_CONNECTIVITY_CRITERION,
    NIDM_HAS_MAXIMUM_INTENSITY_PROJECTION,
    NIDM_HEIGHT_CRITICAL_THRESHOLD_FDR_05,
    NIDM_HEIGHT_CRITICAL_THRESHOLD_FWE_05,
    NIDM_HEIGHT_THRESHOLD,
    NIDM_IN_COORDINATE_SPACE,
    NIDM_INFERENCE,
    NIDM_MAX_NUMBER_OF_PEAKS_PER_CLUSTER,
    NIDM_MIN_DISTANCE_BETWEEN_PEAKS,
    NIDM_NOISE_FWHM_IN_UNITS,
    NIDM_NOISE_FWHM_IN_VOXELS,
    NIDM_NOISE_ROUGHNESS_IN_VOXELS,
    NIDM_NUMBER_OF_CLUSTERS,
    ...
    NIDM_PEAK,
    NIDM_PEAK_DEFINITION_CRITERIA,
    NIDM_Q_VALUE_FDR,
    NIDM_RANDOM_FIELD_STATIONARITY,
    NIDM_RESEL_SIZE_IN_VOXELS,
    NIDM_SEARCH_SPACE_MASK_MAP,
    NIDM_SEARCH_VOLUME_IN_RESELS,
    NIDM_SEARCH_VOLUME_IN_UNITS,
    NIDM_SEARCH_VOLUME_IN_VOXELS,
    NIDM_SIGNIFICANT_CLUSTER,
    NIDM_USER_SPECIFIED_THRESHOLD_TYPE,
    NIDM_VOXEL6CONNECTED,
...
prefix spm_PartialConjunctionDegree: <http://purl.org/nidash/spm#SPM_0000015>
    ...
    def export(self, nidm_version, export_dir):
        ...
        if self.partial_degree is not None:
            atts += ((SPM_PARTIAL_CONJUNCTION_DEGREE, self.partial_degree),)
...
class HeightThreshold(NIDMObject):
    ...
    def __init__(
        ...
        if (
            not stat_threshold
            and not p_corr_threshold
            and not p_uncorr_threshold
            and not value
        ):
        ...
        if stat_threshold is not None:
            ...
            thresh_desc = "p<" + str(p_uncorr_threshold) + " (uncorrected)"
            if version["num"] == "1.0.0":
    ...
    def export(self, version, export_dir):
        ...
        if self.equiv_thresh is not None:
            for equiv in self.equiv_thresh:
                atts += [(NIDM_EQUIVALENT_THRESHOLD, equiv.id)]
...
prefix nidm_clusterSizeInResels: <http://purl.org/nidash/nidm#NIDM_0000156>
    ...
    def export(self, version, export_dir):
        ...
        if version["num"] == "1.0.0":
            ...
            atts += [(PROV["type"], self.threshold_type)]
            if self.value is not None:
                atts += [(PROV["value"], self.value)]
...
class SearchSpace(NIDMObject):
    ...
    def __init__(
        ...
        vol_in_units,
        vol_in_resels,
        resel_size_in_voxels,
        random_field_stationarity,
        noise_fwhm_in_voxels,
        noise_fwhm_in_units,
        coord_space,
        expected_num_voxels=None,
        expected_num_clusters=None,
        height_critical_fwe05=None,
        height_critical_fdr05=None,
        extent_critical_fwe05=None,
        extent_critical_fdr05=None,
        search_vol_geom=None,
    ...
    def export(self, version, export_dir):
        ...
        if self.noise_fwhm_in_voxels is not None:
            if (version["major"] > 1) or (
                version["major"] >= 1
                and (version["minor"] > 0 or version["revision"] > 0)
            ):
                atts = atts + (
        ...
        if self.expected_num_clusters is not None:
            atts = atts + (
                (NIDM_EXPECTED_NUMBER_OF_CLUSTERS, self.expected_num_clusters),
            )
